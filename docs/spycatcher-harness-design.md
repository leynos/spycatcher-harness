...
and a `MatchMode` at construction, extracting interaction data for efficient
matching.
...
- The observed request hash (pre-computed by the canonical module).
- The observed canonical request JSON value.

Because the engine owns the cassette, callers do not pass a cassette reference
to `next_match`.

The method returns a `MatchOutcome<'_>` enum:

- `Matched(&'a Interaction)` — the incoming request matched a recorded
  interaction.
- `Mismatch(MismatchDiagnostic)` — no match was found; diagnostics explain
  why.
...
class without string-matching free-form text:

- `DIAGNOSTIC_EXHAUSTED` (`"cassette-exhausted"`) — the cassette has no
  more interactions to serve.
...
The diff summary body (after the prefix) is generated by
`canonical_diff_summary` in `src/cassette/diff.rs`, which walks two
`serde_json::Value` trees and reports:

- `added: <path>: <value>` — field present in observed but not expected.
- `removed: <path>` — field present in expected but not observed.
- `changed: <path>: <expected_value> -> <observed_value>` — differing
  values.

Paths use dotted notation for nested objects (`canonical_body.metadata.run_id`)
and bracket notation for array elements (`messages[0].role`).
