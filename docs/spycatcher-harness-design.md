...
`src/cassette/matching.rs`. The engine is constructed from a loaded `Cassette`
and a `MatchMode`, and provides a `next_match` method that accepts:

- The observed request hash (pre-computed by the canonical module).
- The observed canonical request JSON value.
- A reference to the cassette.

The `next_match` method returns a `MatchOutcome<'_>` enum:

- `Matched(&'a Interaction)` — the incoming request matched a recorded
  interaction.
- `Mismatch(MismatchDiagnostic)` — no match was found; diagnostics explain why.
...
The diff summary is generated by `canonical_diff_summary` in
`src/cassette/diff.rs`, which walks two `serde_json::Value` trees and reports:
